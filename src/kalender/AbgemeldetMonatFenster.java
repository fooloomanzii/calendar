...
public class AbgemeldetMonatFenster {
	...
	private void initialize() {
		...
		JMenuItem mntmAnmelden = new JMenuItem("Anmelden");
		mntmAnmelden.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				AnmeldungsFenster fen = new AnmeldungsFenster();
				fen.setVisible(true);
				frame.setVisible(false);
			}
		});
		mnMen.add(mntmAnmelden);
		...
		for(int eins=1; eins<=31; eins++){
			...
			for(int i=0; i<termine.size(); i++){
				if(aktuellesDatum.equals(termine.get(i).getDatumVon())){
					...
					hallo.setAlignmentX(Component.CENTER_ALIGNMENT);
					liste.get(start).add(hallo, "alignx center");
