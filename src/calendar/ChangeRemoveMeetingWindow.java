...
public class ChangeRemoveMeetingWindow extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public ChangeRemoveMeetingWindow(final Meeting meeting) {
		...
		dateFromText.setText(meeting.getDateFrom());
		...
		dateToText.setText(meeting.getDateTo());
		...
		timeFromText.setText(meeting.getTimeFrom());
		...
		timeToText.setText(meeting.getTimeTo());
		...
		int i = 0;
		if (meeting.getRepeat().equals("Daily")){
			i = 1;
		}
		if (meeting.getRepeat().equals("Weekly")){
			i = 2;
		}
		if (meeting.getRepeat().equals("Monthly")){
			i = 3;
		}
		if (meeting.getRepeat().equals("Yearly")){
			i = 4;
		}
		repeatTextBox.setSelectedIndex(i);
		...
		repeatToText.setText(meeting.getRepeatTo());
		...
		int j = 0;
		if (meeting.getVisibility().equals("privat")){
			j = 1;
		}
		visibilityBox.setSelectedIndex(j);
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if(title.equals("") || dateFrom.equals("") || dateTo.equals("") || timeFrom.equals("") || timeTo.equals("")){
					...
					if (repeat.equals("Unique")){
						Meeting.changeMeeting(meeting, title, dateFrom, dateTo, timeFrom, timeTo, location, description, repeat, repeatTo, visibility);
						dispose();
					}
					if (repeat.equals("Daily")){dispose();
					}
					if (repeat.equals("Weekly")){dispose();
					}
					if (repeat.equals("Monthly")){dispose();
					}
					if (repeat.equals("Yearly")){dispose();
					}
				}
			}
		});
