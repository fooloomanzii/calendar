...
public class LoginCalendarWindow {
	...
	private int currentDay = _calendar.get(Calendar.DAY_OF_MONTH); // Indicates
																	// current
																	// day
	private int currentMonth = _calendar.get(Calendar.MONTH); // Indicates
																// current day
	private int currentYear = _calendar.get(Calendar.YEAR); // Indicates current
															// year
	...
	private void initializeMonthView() {
		...
		gbc_after.insets = new Insets(4, 4, 6, 6);
		gbc_after.gridx = 6;
		gbc_after.gridy = 0;
		...
		mntmManageUserData.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// JOptionPane.showMessageDialog(frame.getContentPane(), "Work
				// in progress!");
				UserDataWindow win = new UserDataWindow(loginPerson);
				win.setVisible(true);
				frame.setVisible(false);
			}
		});
	...
	private void dayInitializeMonth(int number) {
		...
		for (int i = 0; i < listJPanel.size(); i++) {
			if (i >= start && i < _calendar.getActualMaximum(Calendar.DATE) + start) {
				...
				if (day == currentDay && currentMonth == _calendar.get(Calendar.MONTH)
